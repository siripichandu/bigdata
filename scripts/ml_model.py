...
def build_ml_dataset(df_intakes: pd.DataFrame, df_outcomes: pd.DataFrame) -> pd.DataFrame:
    ...
    intake_cols  = ["Animal ID", "Animal Type", "Intake Type", "Intake Condition",
                    "Age_Months", "Sex", "Neutered", "Mixed_Breed", "Month", "Hour", "DayOfWeek"]
    outcome_cols = ["Animal ID", "Outcome Type", "Positive_Outcome", "Outcome_Group"]
    ...
    # Drop rows where the animal came back multiple times — keep first outcome
...
def evaluate_models(trained, X_test, y_test, feature_cols):
    ...
    for name, info in trained.items():
        ...
        results[name] = {**info, "acc": acc, "auc": auc, "ap": ap,
...
def plot_ml_results(results, best_name, X_test, y_test, feature_cols):
    ...
    plt.suptitle("ML Model Evaluation\nPredicting Positive Outcome (Adoption / Return to Owner)",
                 fontsize=15, fontweight="bold", y=1.01)
    plt.tight_layout()
    path = CHARTS_DIR + "07_ml_evaluation.png"
...
if __name__ == "__main__":
    ...
    results, best_name = evaluate_models(trained, X_test, y_test, feature_cols)
    plot_ml_results(results, best_name, X_test, y_test, feature_cols)
    save_best_model(results, best_name, le_dict, feature_cols)
